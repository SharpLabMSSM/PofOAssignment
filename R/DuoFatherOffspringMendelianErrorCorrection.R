#' @title
#' Step 1 of 3 : Mendel Error detection in father-offspring Duos and set it to missing
#' @description
#' Scan each father-offspring duo for Mendel errors and set such sites to missing in all family members using parallel computing
...
#' @examples
#' DuoFatherOffspringMendelError("/my/dir/data/myphased.vcf","/my/dir/data/myped.txt",2)
#' DuoFatherOffspringMendelError("/my/dir/data/myphased.vcf","/my/dir/data/myped.txt")
#' @note
#' Automatically creates dir named 'MendelErrorFixed/FatherOffsprings' one level above the data dir for the result file of this process
#' @export
#' @author Bharati Jadhav
DuoFatherOffspringMendelError = function(vcf, ped, ncore){
  mainTic = Sys.time()
  ###checking arguments
  if (missing(vcf))
  ...
  outDir=paste(dirname(inDir),"MendelErrorFixed","FatherOffsprings",sep="/")
  ...
  print(paste("No. of full father-offsprings duos : ",nrow(matchduo)))
  if(nrow(matchduo) < 1){
  ...
  print("Finding Mendel error in father-offsprings duos....")
  pofo1=pofo
  tic = Sys.time()
