#' @title
#' Step 2 of 3: Sliding window algorithm to detect and fix phase switch error in father-offspring duos
#' @description
#' Scan each father-offpsring duo for to detect and fix phase switch error in offspring using sliding window (size 100 and step 50 sites) using parallel computing
...
#' @examples
#' DuoFatherOffspringPhaseCorrect("/my/dir/MendelErrorFixed/FatherOffsprings/myphased.MendelErrorFixed.vcf","/my/dir/data/myped.txt",2)
#' DuoFatherOffspringPhaseCorrect("/my/dir/MendelErrorFixed/FatherOffsprings/myphased.MendelErrorFixed.vcf","/my/dir/data/myped.txt")
#' @author Bharati Jadhav
#' @export
#' @note
#' Automatically creates dir named 'PhaseSwitchSlidingWindow/FatherOffsprings' one level above the data dir for the result file of this process
DuoFatherOffspringPhaseCorrect = function(vcf, ped, ncore){
    mainTic = Sys.time()
    ###checking arguments
    if (missing(vcf))
    ...
    inDir=dirname(vcf)
    outDir=paste(dirname(dirname(inDir)),"PhaseSwitchSlidingWindow","FatherOffsprings",sep="/")
    inVcfFile=vcf
    pedFile=ped
    ...
    fidx=match(duoped1$PATID, colnames(duovcf))
    matchduo=na.omit(cbind(iidx,fidx))
    print(paste("No. of father offspring duos : ",nrow(matchduo)))
    ...
    #################################################
    print("Finding Phase switch error for father-offsprings pairs....")
    pofo1=pofo
    tic = Sys.time()
    #open and read the duos which has switch error
    print("Finding Phase switch error for father-offsprings duos....")
    pofo=foreach(i = 1:nrow(matchduo), .combine='cbind',.inorder=TRUE) %dopar% {
        ...
        colnames(haplo1)=c("WindowNo","C1DA1","C1DA2")
        haplo2 = matrix(0, nrow=1, ncol = 3, byrow=T)
        colnames(haplo2)=c("WindowNo","C2DA1","C2DA2")
        from=wSize
        startIdx=1
        endIdx=wSize
        to=nrow(duo)+(wSize-1)
        ...
        while(startIdx <= nrow(duovcf))
        {
            ...
            PatA1=substr(duo[rows,11],1,1)
            PatA2=substr(duo[rows,11],3,3)
            A1Winscore = c(startIdx,sum(CHA1==PatA1),sum(CHA1==PatA2))
            ...
            if(nrow(A1WFilter)>2){
                ...
                #srd1 = ifelse(length(grep("DA", rownames(A1WFilter))) > 1,"C1DA",ifelse(length(grep("MA", rownames(A1WFilter))) > 1,"C1MA1","NA"))
                srd1 = ifelse(length(grep("DA", rownames(A1WFilter))) > 1,"C1DA1","NA")
            }else if(nrow(A1WFilter)==2){
                srd1=rownames(A1WFilter)[2]
            }else {srd1="NA"}

            A2Winscore = c(startIdx,sum(CHA2==PatA1),sum(CHA2==PatA2))
            ...
            if(nrow(A2WFilter)>2){
                #srd2 = ifelse(length(grep("DA", rownames(A2WFilter))) > 1,"C2DA",ifelse(length(grep("MA", rownames(A2WFilter))) > 1,"C2MA1","NA"))
                srd2 = ifelse(length(grep("DA", rownames(A2WFilter))) > 1,"C2DA2","NA")
            }else if(nrow(A2WFilter)==2){
                srd2=rownames(A2WFilter)[2]
            }else {srd2="NA"}

          if((srd1 == "NA") & (srd2 == "C2DA1" | srd2 == "C2DA2" )){
              op[r,2]=paste("DA",CHA2[1:step],sep=":")
              op[r,1]= paste("MA",CHA1[1:step],sep=":")
          }else if((srd1 == "C1DA1" | srd1 == "C1DA2") & ( srd2 == "NA")){
              op[r,2]= paste("DA",CHA1[1:step],sep=":")
              op[r,1]=paste("MA",CHA2[1:step],sep=":")
          }else if((srd1 == "C1DA1" | srd1 == "C1DA2") & ( srd2 == "C2DA1" | srd2 == "C2DA2")){
              op[r,1]= paste("MA",CHA1[1:step],sep=":")
              op[r,2]=paste("DA",CHA2[1:step],sep=":")
          }else{ op[r,]= "." }
    ...
    print ("Time for process samples parallely")
    toc=Sys.time()
    print(toc-tic)
