#' @title
#' Step 3 of 3 : Recover het sites in offsprings set to missing during sliding window (step2) in mother-offspring Duos
#' @description
...
DuoMotherOffspringRecoverMissing = function(vcf1,vcf2, ped){
    mainTic = Sys.time()
    ###checking arguments
    if (missing(vcf1))
    ...
    inDir2=dirname(vcf2)
    outDir=paste(dirname(dirname(inDir2)),"PhaseSwitchMissingFix","MotherOffsprings",sep="/")
    inVcfFile1=vcf1
    inVcfFile2=vcf2
    pedFile=ped
    ...
    midxSource=match(duoped1$MATID, colnames(sourceVcf))
    matchduoSource=na.omit(cbind(iidxSource,midxSource))
    ...
    print("Ordering source vcf in pedigree order [child,mother]  .....")
    ...
    midxTarget=match(duoped1$MATID, colnames(targetVcf))
    matchduoTarget=na.omit(cbind(iidxTarget,midxTarget))
	...
	hetCtAfter=0
        ...
        duoTarget[fixrows,10]= ifelse(duoSO[fixrows,11] == "0|0","0|1",ifelse(duoSO[fixrows,11] == "1|1","1|0",".|."))
